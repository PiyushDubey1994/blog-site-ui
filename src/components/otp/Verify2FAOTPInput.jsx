...
const Verify2FAOTPInput = ({length = 6, onChange, uemail,type}) => {
    console.log("verfy otp "+uemail);
    ...
    const handleSubmit = async () => {
        if (otpCode.length === length && type==="new") {
            try {
                console.log(data1);
                console.log("fghfjfjh"+authData.accessToken)
                const response = await axios.post('http://34.70.118.87:80/api/v1.0/blogsite/auth/user/2fa-enable', data1,{
                    headers: {
            ...
            try {
                console.log(data1);
                console.log("fghfjfjh"+authData.accessToken)
                const response = await axios.post('http://34.70.118.87:80/api/v1.0/blogsite/auth/user/2fa-verify', data1,{
                    headers: {
                      Authorization: `Bearer ${authData.accessToken}`
                    }
                  });
                ...
                console.error('Verify Entered wrong verifying OTP:', error);
            }
...
export default Verify2FAOTPInput;
